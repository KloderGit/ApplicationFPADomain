...
namespace WebApiBusinessLogic.Logics.CallBack
{
        ...
        public async Task<bool> CreateRecallTask(CallBackDTO model)
        {
            ...
            if (contact != null)
            {
                ...
                var modelTask = new TaskDTO()
                {
                    ...
                    Text = "[ ТЕСТОВАЯ ЗАДАЧА ]" + "Клиент заказал обратный звонок!" + "\r\n"
                ...
                try
                {
                    System.Threading.Tasks.Task taskAddTask = System.Threading.Tasks.Task.Factory.StartNew(
                       () => {
                           crm.Tasks.Add(new[] { modelTask });
                       }
                    );
                }
                catch (Exception ex)
